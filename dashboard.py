# dashboard.py - الكود الكامل مع مسار ثابت لملف CSV للتنبؤات (مُصحح وبدون ضغط)
...
FORECAST_CSV_PATH = "future_forecasts_standalone.csv" # مثال: "C:/Users/YourUser/Documents/forecast.csv" أو "forecast_data.csv"
# ----------------------------------------------------
...
class DataPipeline:
    ...
    def load_original_data(_self):
        ...
        if 'products' not in raw_data_loaded or raw_data_loaded['products'].empty:
             st.warning("تحذير: ملف المنتجات مفقود أو فارغ. بعض التحليلات قد لا تعمل.")
        ...
        if missing_essentials:
             st.error(f"الملفات الأساسية للفواتير مفقودة: {', '.join(missing_essentials)}. لا يمكن المتابعة.")
             st.stop()
    ...
    def preprocess_and_analyze(_self, raw_data):
        ...
        prod_buy_price_col_p = next((col for col in ['buyPrice', 'cost'] if col in products.columns), None) # Renamed to avoid conflict
        prod_sale_price_col_p = next((col for col in ['salePrice', 'price'] if col in products.columns), None) # Renamed to avoid conflict
        required_missing = []
        check_cols = {'si_date_col': si_date_col, 'si_id_col': si_id_col, 'sid_invoice_id_col': sid_invoice_id_col, 'sid_amount_col': sid_amount_col, 'sid_quantity_col': sid_quantity_col, 'sid_product_id_col': sid_product_id_col}
        ...
        if any(c.startswith("عمود") for c in required_missing):
             st.error("الأعمدة الأساسية للفواتير مفقودة: " + ", ".join(required_missing))
             return processed_data, analytics
        elif required_missing:
             st.warning("بعض الأعمدة/الملفات مفقودة: " + ", ".join(required_missing))
        try:
            ...
            si[si_total_col_original] = pd.to_numeric(si.get(si_total_col_original), errors='coerce').fillna(0).astype('float32')
            ...
            sid[sid_amount_col_original] = pd.to_numeric(sid.get(sid_amount_col_original), errors='coerce').fillna(0).astype('float32')
            sid_quantity_col_original = sid_quantity_col
            sid[sid_quantity_col_original] = pd.to_numeric(sid.get(sid_quantity_col_original), errors='coerce').fillna(0).astype('float32')
            sid_buy_price_col_original = sid_buy_price_col
            # Handle potential missing buy price column gracefully
            if sid_buy_price_col_original and sid_buy_price_col_original in sid.columns:
                ...
                sid['buyPrice'] = 0.0 # Assign default if missing
                sid_buy_price_col_original = 'buyPrice' # Use default name
            ...
            if not products.empty:
                ...
                # Use renamed variable prod_buy_price_col_p
                if prod_buy_price_col_p and prod_buy_price_col_p in products.columns:
                 ...
                 # Use renamed variable prod_sale_price_col_p
             ...
             processed_data={'sale_invoices': si,'sale_invoices_details': sid,'products': products,'invoice_deferred': deferred}
             return processed_data, analytics

        try:
            merged_sales_fig11 = pd.merge(sid[['invoice_fk', sid_amount_col_original]], si[['invoice_pk', 'created_at_per_day']], left_on='invoice_fk', right_on='invoice_pk', how='left')
            ...
            if not products.empty and not sid.empty:
                ...
                product_flow_df = pd.merge(sales_qty_agg.rename(columns={sid_quantity_col_original: 'sales_quantity'}), sales_total_agg.rename(columns={sid_amount_col_original: 'sales_amount'}), on='product_id', how='outer').fillna(0)
                ...
                product_flow_df['efficiency'] = pd.cut(product_flow_df['efficiency_ratio'], bins=[0, 0.8, 1.2, float('inf')], labels=['Undersupplied', 'Balanced', 'Oversupplied'], right=False)
                ...
                if 'created_at_dt' in sid.columns and not sid['created_at_dt'].isnull().all():
                     ...
                     product_flow_df.loc[mask_valid_date, 'days_since_last_sale'] = (today_ts - product_flow_df.loc[mask_valid_date, 'last_sale_date'].dt.normalize()).dt.days
                     product_flow_df['days_since_last_sale'] = pd.to_numeric(product_flow_df['days_since_last_sale'], errors='coerce').fillna(9999).astype(int)
                else:
            ...
            if not analytics['product_flow'].empty:
                ...
                if total_sales_quantity > 0:
                    ...
                    sorted_products['cumulative_percentage'] = 0
                ...
                analytics['pareto_data'] = pd.DataFrame()
            ...
            if not analytics['product_flow'].empty:
                ...
                analytics['pie_data'] = {'revenue': pd.DataFrame(), 'profit': pd.DataFrame(), 'color_mapping': {}}
            ...
            if not analytics['product_flow'].empty and 'days_since_last_sale' in analytics['product_flow'].columns:
                 ...
                 stagnant = analytics['product_flow'][(days_col >= 90) & (days_col != 9999)].copy()
                 if not stagnant.empty:
                     ...
                     if not stagnant.empty: # Check again after dropna
                         stagnant['days_category'] = pd.cut(stagnant['days_since_last_sale_num'], bins=[90, 270, 365, float('inf')], labels=['3-9 أشهر', '9-12 شهر', 'أكثر من سنة'], right=False)
                         ...
                         analytics['stagnant_products'] = pd.DataFrame() # Empty after dropna
                 else:
                     analytics['stagnant_products'] = pd.DataFrame() # Empty initially
            else:
                analytics['stagnant_products'] = pd.DataFrame()

            if not deferred.empty:
                def_type_col = 'invoice_type'; def_status_col = 'status'; def_amount_col = 'amount'; def_paid_col = 'paid_amount'; def_user_id_col = 'user_id'
                ...
                if all(col in deferred.columns for col in required_deferred_cols):
                    ...
                    status_values = [0, 2]
                    filtered = deferred[(deferred[def_type_col] == buy_invoice_type_str) & (deferred[def_status_col].isin(status_values))].copy()
                    if not filtered.empty:
                        ...
                        analytics['outstanding_amounts'] = pd.DataFrame() # No matching deferred invoices
                else:
    ...
    def load_forecast_csv(self, file_path):
        if not file_path:
            st.warning("لم يتم تحديد مسار ملف CSV للتنبؤات (FORECAST_CSV_PATH).")
            self.forecast_data_from_csv = pd.DataFrame()
            return False
        if not os.path.exists(file_path):
            st.error(f"ملف CSV للتنبؤات غير موجود في المسار المحدد: {file_path}")
            self.forecast_data_from_csv = pd.DataFrame()
            return False
        try:
            ...
            if missing_required_cols:
                st.error(f"ملف CSV للتنبؤ في المسار '{file_path}' يفتقد للأعمدة الأساسية المطلوبة: {', '.join(missing_required_cols)}")
                return False
            try:
                df[DATE_COLUMN_CSV] = pd.to_datetime(df[DATE_COLUMN_CSV])
            except Exception as e_date:
                st.error(f"خطأ في تحويل عمود التاريخ '{DATE_COLUMN_CSV}' في ملف CSV من المسار '{file_path}': {e_date}")
                return False
            numeric_cols_csv = [FORECAST_COLUMN_CSV]
            optional_csv_cols = [LOWER_CI_COLUMN_CSV, UPPER_CI_COLUMN_CSV]
            ...
            for col in numeric_cols_csv:
                if col in df.columns:
                    ...
                    if df[col].isnull().any() and not pd.api.types.is_numeric_dtype(original_type):
                         st.warning(f"تحذير (CSV): تم العثور على قيم غير رقمية في عمود '{col}' في الملف '{file_path}' وتم تحويلها إلى NaN.")
            df = df.sort_values(by=DATE_COLUMN_CSV).reset_index(drop=True)
            self.forecast_data_from_csv = df
            return True
        except Exception as e:
            st.error(f"حدث خطأ أثناء قراءة أو معالجة ملف CSV من المسار '{file_path}': {e}")
            self.forecast_data_from_csv = pd.DataFrame()
            return False
    ...
    def generate_visualizations(self):
        ...
        sid_data = self.processed_data.get('sale_invoices_details', pd.DataFrame())
        ...
        if not product_flow.empty:
            ...
            if not top20_qty.empty:
                ...
                fig1.update_layout(title=fig1_title).add_annotation(text="لا توجد بيانات Top 20", showarrow=False)
            self.visualizations['fig1'] = fig1
        else:
            self.visualizations['fig1'] = go.Figure().update_layout(title=fig1_title).add_annotation(text="بيانات تدفق المنتجات مفقودة", showarrow=False)
        ...
        if not product_flow.empty and 'efficiency' in product_flow.columns:
            ...
            if not efficiency_counts.empty:
                fig2 = px.pie(values=efficiency_counts.values, names=efficiency_counts.index, title=fig2_title, color_discrete_sequence=['#1f77b4', '#ff7f0e', '#2ca02c'])
                self.visualizations['fig2'] = fig2
            else:
                self.visualizations['fig2'] = go.Figure().update_layout(title=fig2_title).add_annotation(text="لا توجد بيانات كفاءة صالحة", showarrow=False)
        else:
            self.visualizations['fig2'] = go.Figure().update_layout(title=fig2_title).add_annotation(text="بيانات الكفاءة مفقودة", showarrow=False)

        # --- fig3 ---
        if not product_flow.empty and all(c in product_flow.columns for c in ['buyPrice', 'sales_quantity', 'sales_amount', 'name']):
            ...
            if not top20_amount.empty:
                ...
                fig3 = px.bar(top20_amount, x="name", y=["COGS", "sales_amount", "margin"], barmode="group", title=fig3_title, labels={'value': 'المبلغ', 'variable': 'النوع', 'name':'المنتج'}, color_discrete_map={"COGS": "blue", "sales_amount": "orange", "margin": "green"})
                self.visualizations['fig3'] = fig3
            else:
                self.visualizations['fig3'] = go.Figure().update_layout(title=fig3_title).add_annotation(text="لا توجد بيانات Top 20", showarrow=False)
        else:
            self.visualizations['fig3'] = go.Figure().update_layout(title=fig3_title).add_annotation(text="بيانات غير كافية", showarrow=False)

        # --- fig4 ---
        if not product_flow.empty:
            top20_qty_fig4 = pd.DataFrame() # Initialize
            if 'fig1' in self.visualizations and self.visualizations['fig1'].data and hasattr(self.visualizations['fig1'].data[0], 'x'):
                top20_names_fig1 = self.visualizations['fig1'].data[0].x
                if top20_names_fig1 is not None: # Ensure names were actually extracted
                     top20_qty_fig4 = product_flow[product_flow['name'].isin(top20_names_fig1)].nlargest(20, 'sales_quantity')
            if top20_qty_fig4.empty: # Fallback if names not found or filter results in empty
                top20_qty_fig4 = product_flow.nlargest(20, 'sales_quantity')
            ...
            if not top20_qty_fig4.empty:
                ...
                self.visualizations['fig4'] = go.Figure().update_layout(title=fig4_title).add_annotation(text="لا توجد بيانات Top 20", showarrow=False)
        else:
            self.visualizations['fig4'] = go.Figure().update_layout(title=fig4_title).add_annotation(text="بيانات تدفق المنتجات مفقودة", showarrow=False)
        ...
        if not product_flow.empty and 'efficiency' in product_flow.columns:
             ...
             if not eff_group.empty:
                  ...
                  self.visualizations['fig5'] = go.Figure().update_layout(title=fig5_title).add_annotation(text="لا توجد بيانات كفاءة صالحة", showarrow=False)
        else:
             self.visualizations['fig5'] = go.Figure().update_layout(title=fig5_title).add_annotation(text="بيانات الكفاءة مفقودة", showarrow=False)

        # --- fig6 ---
        pareto_data = self.analytics.get('pareto_data', pd.DataFrame())
        fig6 = go.Figure()
        if not pareto_data.empty and all(c in pareto_data.columns for c in ['name', 'sales_quantity', 'cumulative_percentage', 'category']):
            fig6.add_trace(go.Bar(x=pareto_data['name'], y=pareto_data['sales_quantity'], name="المبيعات", marker_color='blue'))
            colors = px.colors.qualitative.Plotly
            valid_categories = sorted([cat for cat in pareto_data['category'].unique() if pd.notna(cat)])
            for i, category in enumerate(valid_categories):
                cat_data = pareto_data[pareto_data['category'] == category]
                if not cat_data.empty:
                    fig6.add_trace(go.Scatter(x=cat_data['name'], y=cat_data['cumulative_percentage'], name=f"{int(category)}-{int(category + 10)}%", mode='lines+markers', line=dict(color=colors[i % len(colors)], dash='dash'), yaxis="y2"))
            fig6.update_layout(title=fig6_title, xaxis_title="المنتج", yaxis_title="المبيعات", yaxis2=dict(title="النسبة التراكمية (%)", overlaying="y", side="right", range=[0, 100], showgrid=False), legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
            self.visualizations['fig6'] = fig6
        else:
            self.visualizations['fig6'] = go.Figure().update_layout(title=fig6_title).add_annotation(text="بيانات باريتو غير متوفرة", showarrow=False)

        # --- fig7 ---
        if not product_flow.empty and all(c in product_flow.columns for c in ['salePrice', 'sales_quantity', 'name']):
             fig7_data = product_flow[(product_flow['salePrice'] > 0) & (product_flow['sales_quantity'] > 0)]
             if not fig7_data.empty:
                 fig7 = px.scatter(fig7_data, x="salePrice", y="sales_quantity", title=fig7_title, labels={'salePrice': 'سعر البيع', 'sales_quantity': 'حجم المبيعات'}, trendline="lowess", hover_data=['name'])
                 self.visualizations['fig7'] = fig7
             else:
                 self.visualizations['fig7'] = go.Figure().update_layout(title=fig7_title).add_annotation(text="لا توجد منتجات بأسعار وكميات موجبة", showarrow=False)
        else:
             self.visualizations['fig7'] = go.Figure().update_layout(title=fig7_title).add_annotation(text="بيانات السعر/الكمية مفقودة", showarrow=False)

        # --- fig9 ---
        if not product_flow.empty and all(c in product_flow.columns for c in ['current_stock', 'name']):
            restock = product_flow[product_flow['current_stock'] <= 10].sort_values('current_stock')
            if not restock.empty:
                 fig9 = px.bar(restock, x="name", y="current_stock", title=fig9_title, labels={'name': 'المنتج', 'current_stock': 'المخزون الحالي'})
                 fig9.update_layout(xaxis={'categoryorder':'total ascending'})
                 self.visualizations['fig9'] = fig9
            else:
                 self.visualizations['fig9'] = go.Figure().update_layout(title=fig9_title).add_annotation(text="لا توجد منتجات تحتاج إعادة تخزين", showarrow=False)
        else:
             self.visualizations['fig9'] = go.Figure().update_layout(title=fig9_title).add_annotation(text="بيانات المخزون مفقودة", showarrow=False)
             ...
             if all(c in stagnant_products_for_plot.columns for c in required_cols_fig10):
                 try:
                     ...
                     if not stagnant_products_for_plot.empty:
                         ...
                         self.visualizations['fig10'] = go.Figure().update_layout(title=fig10_title).add_annotation(text="لا توجد بيانات راكدة صالحة للعرض", showarrow=False)
                  ...
                  self.visualizations['fig10'] = go.Figure().update_layout(title=fig10_title).add_annotation(text="بيانات غير كافية", showarrow=False)
        else:
             self.visualizations['fig10'] = go.Figure().update_layout(title=fig10_title).add_annotation(text="لا توجد منتجات راكدة", showarrow=False)

        # --- fig11 ---
        daily_sales_actual_df_fig11 = self.analytics.get('daily_sales_actual', pd.DataFrame())
        future_forecast_df_fig11 = self.forecast_data_from_csv
        fig11 = go.Figure()
        last_actual_point_details_fig11 = None
        if isinstance(daily_sales_actual_df_fig11, pd.DataFrame) and not daily_sales_actual_df_fig11.empty:
            try:
                ...
                if not df_actual_plot.empty:
                    last_actual_row = df_actual_plot.iloc[-1]
                    last_actual_point_details_fig11 = {'date': last_actual_row['date'], 'actual': last_actual_row['actual']}
            ...
            except Exception as e_plot_actual_fig11:
                st.warning(f"خطأ رسم فعلية fig11: {e_plot_actual_fig11}")
                last_actual_point_details_fig11 = None
        if isinstance(future_forecast_df_fig11, pd.DataFrame) and not future_forecast_df_fig11.empty:
            forecast_to_plot = future_forecast_df_fig11.copy()
            if DATE_COLUMN_CSV in forecast_to_plot.columns:
                forecast_to_plot[DATE_COLUMN_CSV] = pd.to_datetime(forecast_to_plot[DATE_COLUMN_CSV])
            else:
                st.warning(f"عمود التاريخ '{DATE_COLUMN_CSV}' مفقود في بيانات التنبؤ CSV.")
                forecast_to_plot = pd.DataFrame()
            if FORECAST_COLUMN_CSV in forecast_to_plot.columns and not forecast_to_plot.empty:
                try:
                    forecast_to_plot = forecast_to_plot.sort_values(DATE_COLUMN_CSV)
                    connected_forecast_df = forecast_to_plot
                    if last_actual_point_details_fig11 is not None:
                        last_actual_dt = last_actual_point_details_fig11['date']
                        last_actual_val = last_actual_point_details_fig11['actual']
                        forecast_future_only = forecast_to_plot[forecast_to_plot[DATE_COLUMN_CSV] > last_actual_dt].copy()
                        connection_point_data = {DATE_COLUMN_CSV: last_actual_dt, FORECAST_COLUMN_CSV: last_actual_val, LOWER_CI_COLUMN_CSV: np.nan if LOWER_CI_COLUMN_CSV in forecast_future_only.columns else None, UPPER_CI_COLUMN_CSV: np.nan if UPPER_CI_COLUMN_CSV in forecast_future_only.columns else None}
                        connection_point_data = {k: v for k, v in connection_point_data.items() if k in forecast_future_only.columns or k == DATE_COLUMN_CSV}
                        connection_df = pd.DataFrame([connection_point_data])
                        if not forecast_future_only.empty:
                            connected_forecast_df = pd.concat([connection_df, forecast_future_only], ignore_index=True)
                        else:
                            connected_forecast_df = pd.DataFrame() # لا ترسم إذا لا يوجد مستقبل
                    if not connected_forecast_df.empty:
                        fig11.add_trace(go.Scatter(x=connected_forecast_df[DATE_COLUMN_CSV], y=connected_forecast_df[FORECAST_COLUMN_CSV], mode='lines', name='المبيعات المتوقعة', line=dict(color='rgba(255, 65, 54, 0.9)', dash='dash', width=2)))
                        has_lower_ci_csv = LOWER_CI_COLUMN_CSV in connected_forecast_df.columns
                        has_upper_ci_csv = UPPER_CI_COLUMN_CSV in connected_forecast_df.columns
                        if has_lower_ci_csv and has_upper_ci_csv:
                            ci_valid_csv = connected_forecast_df.iloc[1:].dropna(subset=[LOWER_CI_COLUMN_CSV, UPPER_CI_COLUMN_CSV])
                            if not ci_valid_csv.empty:
                                fig11.add_trace(go.Scatter(x=ci_valid_csv[DATE_COLUMN_CSV].tolist() + ci_valid_csv[DATE_COLUMN_CSV].tolist()[::-1], y=ci_valid_csv[UPPER_CI_COLUMN_CSV].tolist() + ci_valid_csv[LOWER_CI_COLUMN_CSV].tolist()[::-1], fill='toself', fillcolor='rgba(255, 65, 54, 0.15)', line=dict(color='rgba(255,255,255,0)'), name='فترة الثقة 95%', showlegend=True, hoverinfo='skip'))
                except Exception as e_plot_forecast_fig11:
                    st.warning(f"خطأ رسم تنبؤ fig11: {e_plot_forecast_fig11}")
            elif not future_forecast_df_fig11.empty:
                st.warning(f"ملف CSV يفتقد لعمود '{FORECAST_COLUMN_CSV}'.")
        elif last_actual_point_details_fig11 is not None:
             st.info("لم يتم تحميل بيانات التنبؤ CSV لعرضها مع البيانات الفعلية.")
        ...
        all_dates_fig11 = pd.Series(dtype='datetime64[ns]')
        if isinstance(daily_sales_actual_df_fig11, pd.DataFrame) and not daily_sales_actual_df_fig11.empty:
             all_dates_fig11 = pd.concat([all_dates_fig11, pd.to_datetime(daily_sales_actual_df_fig11['date'])])
        if isinstance(future_forecast_df_fig11, pd.DataFrame) and not future_forecast_df_fig11.empty and DATE_COLUMN_CSV in future_forecast_df_fig11.columns:
             all_dates_fig11 = pd.concat([all_dates_fig11, pd.to_datetime(future_forecast_df_fig11[DATE_COLUMN_CSV])])
        if not all_dates_fig11.empty:
            try:
                min_date_fig11 = all_dates_fig11.min()
                max_date_fig11 = all_dates_fig11.max()
                delta_days_fig11 = (max_date_fig11 - min_date_fig11).days if max_date_fig11 > min_date_fig11 else 1
                date_margin_fig11 = timedelta(days=max(7, delta_days_fig11 * 0.05))
                fig11.update_xaxes(range=[min_date_fig11 - date_margin_fig11, max_date_fig11 + date_margin_fig11])
            except Exception as e_xaxis_fig11:
                st.warning(f"خطأ تحديد نطاق X لـ fig11: {e_xaxis_fig11}")
        ...
        if not si_data.empty and all(c in si_data.columns for c in ['month_name', si_total_col_fig12, 'year', 'month']):
            ...
            if not monthly_avg.empty:
                 ...
                 fig12 = px.line(monthly_avg, x="display_month", y=si_total_col_fig12, title=fig12_title, labels={"display_month": "الشهر", si_total_col_fig12: "متوسط قيمة الفاتورة"}, markers=True)
                 fig12.update_xaxes(type='category')
                 self.visualizations['fig12'] = fig12
            else:
                 self.visualizations['fig12'] = go.Figure().update_layout(title=fig12_title).add_annotation(text="لا توجد بيانات شهرية", showarrow=False)
        else:
             self.visualizations['fig12'] = go.Figure().update_layout(title=fig12_title).add_annotation(text="أعمدة مطلوبة مفقودة", showarrow=False)

        # --- fig13 ---
        if not product_flow.empty and all(c in product_flow.columns for c in ['sales_quantity', 'name', 'product_id']):
            ...
            if not sold_products.empty:
                ...
                fig13 = px.bar(bottom10, x="sales_quantity", y="name", orientation='h', title=fig13_title, labels={"name": "المنتج", "sales_quantity": "الكمية المباعة"}, color="sales_quantity", color_continuous_scale=px.colors.sequential.Viridis, hover_data=["product_id"])
                ...
                self.visualizations['fig13'] = go.Figure().update_layout(title=fig13_title).add_annotation(text="لا توجد منتجات تم بيعها", showarrow=False)
        else:
            self.visualizations['fig13'] = go.Figure().update_layout(title=fig13_title).add_annotation(text="بيانات تدفق المنتجات مفقودة", showarrow=False)
             ...
             if not sales_per_product.empty:
                 ...
                 if len(sales_freq.columns) == 2:
                      ...
                      if not fig14_data.empty:
                           ...
                           except Exception as e_ff_fig14:
                               st.error(f"خطأ إنشاء جدول fig14: {e_ff_fig14}")
                               # self.visualizations['fig14'] remains None
        ...
        if not product_flow.empty and all(c in product_flow.columns for c in ['sales_quantity', 'name', 'product_id']):
            ...
            if not top10.empty and top10['sales_quantity'].iloc[0] > 0:
                 fig15 = px.bar(top10, x="sales_quantity", y="name", orientation='h', title=fig15_title, labels={"name": "المنتج", "sales_quantity": "الكمية المباعة"}, color="sales_quantity", color_continuous_scale=px.colors.sequential.Viridis, hover_data=["product_id"])
                 ...
                 self.visualizations['fig15'] = go.Figure().update_layout(title=fig15_title).add_annotation(text="لا توجد بيانات كافية", showarrow=False)
        else:
             self.visualizations['fig15'] = go.Figure().update_layout(title=fig15_title).add_annotation(text="بيانات تدفق المنتجات مفقودة", showarrow=False)

        # --- fig16 ---
        pie_data = self.analytics.get('pie_data')
        if pie_data and isinstance(pie_data, dict) and 'revenue' in pie_data and not pie_data['revenue'].empty and 'profit' in pie_data and not pie_data['profit'].empty and 'color_mapping' in pie_data and isinstance(pie_data['color_mapping'], dict):
            fig16 = make_subplots(rows=1, cols=2, subplot_titles=["الأعلى إيرادًا", "الأعلى ربحًا"], specs=[[{"type": "domain"}, {"type": "domain"}]])
            revenue_to_plot = pie_data['revenue']
            profit_to_plot = pie_data['profit']
            if not revenue_to_plot.empty:
                revenue_colors = [pie_data['color_mapping'].get(n, "gray") for n in revenue_to_plot["name"]]
                fig16.add_trace(go.Pie(labels=revenue_to_plot["name"], values=revenue_to_plot["totalPrice"], name="إيراد", marker=dict(colors=revenue_colors)), row=1, col=1)
            else:
                fig16.add_annotation(text="لا يوجد إيراد", xref="paper", yref="paper", x=0.18, y=0.5, showarrow=False)
            if not profit_to_plot.empty:
                profit_colors = [pie_data['color_mapping'].get(n, "gray") for n in profit_to_plot["name"]]
                fig16.add_trace(go.Pie(labels=profit_to_plot["name"], values=profit_to_plot["netProfit"], name="ربح", marker=dict(colors=profit_colors)), row=1, col=2)
            else:
                fig16.add_annotation(text="لا يوجد ربح", xref="paper", yref="paper", x=0.82, y=0.5, showarrow=False)
        ...
        if not outstanding.empty and all(c in outstanding.columns for c in ['user_id', 'outstanding_amount']):
             ...
             fig17 = px.bar(outstanding_sorted, x="user_id", y="outstanding_amount", color="outstanding_amount", title=fig17_title, labels={"user_id": "المورد", "outstanding_amount": "المبلغ الآجل"}, color_continuous_scale="Inferno")
             fig17.update_coloraxes(colorbar_title="المبلغ الآجل")
             fig17.update_layout(xaxis={'categoryorder': 'total descending', 'type': 'category'})
             fig17.add_annotation(xref="paper", yref="paper", x=0.98, y=0.98, text=f"مجموع الآجل: {round(total_outstanding,2)}", showarrow=False, font=dict(size=18, color="black"), bgcolor="rgba(211,211,211,0.7)", bordercolor="black", borderwidth=1, borderpad=4, align="right")
             self.visualizations['fig17'] = fig17
        else:
             self.visualizations['fig17'] = go.Figure().update_layout(title=fig17_title).add_annotation(text="لا توجد بيانات مبالغ آجلة", showarrow=False)
    ...
    def run_pipeline(self, forecast_csv_path):
        ...
        try:
            ...
            # Stricter check after loading
            if self.raw_data is None or not isinstance(self.raw_data, dict) or \
               'sale_invoices' not in self.raw_data or self.raw_data['sale_invoices'].empty or \
               'sale_invoices_details' not in self.raw_data or self.raw_data['sale_invoices_details'].empty:
                 st.error("فشل تحميل بيانات Excel الأساسية (الفواتير/التفاصيل).")
                 pipeline_success = False # Stop if core invoice data missing
            else:
                 excel_data_loaded = True
        except Exception as e_load:
            st.error(f"فشل تحميل البيانات الأصلية (Excel): {e_load}")
            st.exception(e_load)
            pipeline_success = False

        if pipeline_success: # Only try CSV if Excel basics loaded
             try:
                 csv_data_loaded = self.load_forecast_csv(forecast_csv_path)
                 # It's okay if csv_data_loaded is False, pipeline continues
             except Exception as e_csv_load:
                  st.error(f"حدث خطأ غير متوقع أثناء تحميل ملف CSV: {e_csv_load}")
                  st.exception(e_csv_load)
                  self.forecast_data_from_csv = pd.DataFrame() # Ensure empty on error

        if pipeline_success and excel_data_loaded: # Ensure excel data is valid before analysis
             try:
                 with st.spinner("جاري معالجة بيانات Excel وتحليلها..."):
                 ...
                 if not is_analytics_valid:
                     st.warning("لم يتم حساب بيانات المبيعات الفعلية بنجاح.")
             except Exception as e_analyze_pipe:
                 st.error(f"فشل في معالجة/تحليل بيانات Excel: {e_analyze_pipe}")
                 st.exception(e_analyze_pipe)
                 pipeline_success = False # Stop if analysis fails

        elif not excel_data_loaded and pipeline_success: # Case where excel loading failed but pipeline didn't stop earlier
             st.error("لا يمكن إجراء التحليل بسبب فشل تحميل بيانات Excel.")
             pipeline_success = False

        if pipeline_success: # Generate plots if pipeline hasn't failed critically
             try:
                 if isinstance(self.analytics, dict) and self.analytics: # Check if analytics dict exists
                     with st.spinner("جاري إنشاء الرسوم البيانية..."):
                         self.generate_visualizations()
                     if not self.visualizations:
                         st.warning("لم يتم إنشاء أي رسوم بيانية.")
                 else:
                     st.warning("لا يمكن إنشاء الرسوم البيانية بسبب عدم توفر بيانات التحليل.")
             except Exception as e_viz_pipe:
                 st.error(f"فشل في إنشاء الرسوم البيانية: {e_viz_pipe}")
                 st.exception(e_viz_pipe)
        else:
             st.error("--- فشل خط أنابيب البيانات في إحدى الخطوات الهامة ---")
    ...
    def display_dashboard(self):
        ...
        def get_fig(fig_key, title="N/A"):
            ...
            if fig is None:
                 fig = go.Figure().update_layout(title=f"{title}: بيانات غير متوفرة", xaxis={'visible': False}, yaxis={'visible': False}, annotations=[{'text': "بيانات غير متوفرة", 'xref': "paper", 'yref': "paper",'showarrow': False, 'font': {'size': 16}}])
            return fig
        ...
        with col14:
             fig14_title = self.visualizations.get('fig14_title', "جدول تكرار كميات البيع")
             st.subheader(fig14_title)
             fig14_obj = self.visualizations.get('fig14')
             if fig14_obj:
                 st.plotly_chart(fig14_obj, use_container_width=True)
             else:
                 st.warning("لم يتم إنشاء جدول تكرار المبيعات (Vis 14).")
        with col15: st.plotly_chart(get_fig('fig16', 'الأعلى ربحًا وإيرادًا'), use_container_width=True)
        st.divider()
        st.header("المبالغ الآجلة للموردين")
        st.plotly_chart(get_fig('fig17', 'المبالغ الآجلة للموردين'), use_container_width=True)
        st.divider()
        ...
        if not products_df_disp.empty and all(c in products_df_disp.columns for c in ['id', 'name', 'quantity']):
            current_stock_display = products_df_disp[['id', 'name', 'quantity']].rename(columns={'quantity': 'current_stock', 'id': 'product_id'})
            if not product_flow_df_disp.empty and all(c in product_flow_df_disp.columns for c in ['product_id', 'sales_quantity']):
            ...
            if 'sales_quantity' in current_stock_display.columns:
                display_cols_table.append('sales_quantity')
            if all(col in current_stock_display.columns for col in display_cols_table):
                try:
                    ...
                    column_config_dict = {"product_id": "ID", "name": "المنتج", "current_stock": st.column_config.NumberColumn("المخزون الحالي", format="%.1f")}
                    if 'sales_quantity' in display_cols_table:
                        column_config_dict["sales_quantity"] = st.column_config.NumberColumn("إجمالي المبيعات", format="%.1f")
                    st.dataframe(df_to_display.sort_values('current_stock', ascending=False).style.background_gradient(subset=['current_stock'], cmap='Greens'), height=400, use_container_width=True, column_config=column_config_dict)
                except Exception as e_table_display:
                    st.error(f"خطأ عرض جدول المخزون: {e_table_display}")
            else:
                st.warning("الأعمدة المطلوبة لعرض جدول المخزون غير متوفرة.")
        else:
...
if __name__ == "__main__":
    ...
    try:
        ...
        st.error(f"خطأ فادح: ملف Excel أساسي مفقود: {fnf_error}")
        st.info("يرجى التحقق من مسارات ملفات Excel في بداية الكود.")
    except Exception as e:
