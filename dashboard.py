# dashboard.py - الكود الكامل مع مسار ثابت لملف CSV للتنبؤات
...
FORECAST_CSV_PATH = "future_forecasts_standalone.csv" # مثال: "C:/Users/YourUser/Documents/forecast.csv" أو "forecast_data.csv" إذا كان بنفس المجلد
# ----------------------------------------------------

# --- تعريف أسماء الأعمدة المتوقعة في ملف CSV للتنبؤ ---
# (تأكد من مطابقتها للأعمدة في ملفك المحدد في FORECAST_CSV_PATH)
...
class DataPipeline:
    ...
    # --- مرحلة 1: تحميل البيانات الأصلية (Excel) ---
    ...
    def preprocess_and_analyze(_self, raw_data):
        ...
        if any(c.startswith("عمود") for c in required_missing if "products" not in c and "المنتجات" not in c):
             st.error("الأعمدة/الملفات الأساسية للفواتير مفقودة: " + ", ".join(required_missing))
             return processed_data, analytics
        ...
        try:
            ...
            else: sid['buyPrice'] = 0.0; sid_buy_price_col_original = 'buyPrice'
            ...
            if not products.empty:
                ...
                products['name'] = products['product_name']; products['id'] = products['product_pk']
        ...
        try:
            ...
            if not products.empty and not sid.empty:
                ...
                if 'created_at_dt' in sid.columns and not sid['created_at_dt'].isnull().all():
                     ...
                     today_date = datetime.now().date(); today_ts = pd.Timestamp(today_date)
            ...
            if not analytics['product_flow'].empty:
                ...
                total_revenue = product_flow_for_pie['sales_amount'].sum(); total_profit = product_flow_for_pie['netProfit'].sum()
                ...
                analytics['pie_data'] = {'revenue': pie_revenue_df[pie_revenue_df['totalPrice'] > 0].copy(), 'profit': pie_profit_df[pie_profit_df['netProfit'] > 0].copy(), 'color_mapping': color_map}
            ...
            if not analytics['product_flow'].empty and 'days_since_last_sale' in analytics['product_flow'].columns:
                 ...
                 else: pass # No stagnant products found message handled in plot generation
            ...
            if not deferred.empty:
                ...
                if all(col in deferred.columns for col in required_deferred_cols):
                    ...
                    buy_invoice_type_str = "Stocks\\Models\\BuyInvoice"; status_values = [0, 2]
                    ...
                    else: pass # No relevant deferred invoices found message handled in plot generation
                else:
    ...
    # --- مرحلة 3: تحميل بيانات CSV للتنبؤ (من مسار ثابت) ---
    def load_forecast_csv(self, file_path):
        """تحميل بيانات التنبؤ من مسار ملف CSV ثابت."""
        if not file_path:
    ...
    def generate_visualizations(self):
        ...
        fig1_title = "📊 مقارنة المخزون والمبيعات (Top 20)"; fig2_title = "📈 توزيع الكفاءة"; fig3_title = "💰 تحليل الإيرادات (مع COGS)"; fig4_title = "📦 المخزون الحالي مقابل المبيعات"; fig5_title = "⚖️ مقارنة الكفاءة"; fig7_title = "📈 تحليل تسعير المنتجات"; fig9_title = "📦 منتجات تحتاج لإعادة تخزين (<10)"; fig10_title = '📅 المنتجات الراكدة (90+ يوم)'; fig11_title = "📈 المبيعات اليومية: الفعلية والمتوقعة"; fig12_title = "متوسط الفاتورة الشهرية"; fig13_title = "أقل 10 منتجات مبيعًا"; fig14_title_display = "📊 جدول تكرار كميات البيع"; fig15_title = "أكثر 10 منتجات مبيعًا"; fig16_title = "أعلى 10 منتجات ربحًا وإيرادًا"; fig17_title = "الموردين حسب المبلغ الآجل"
        if not product_flow.empty:
            top20_qty = product_flow.sort_values('sales_quantity', ascending=False).head(20); fig1 = go.Figure()
        ...
        if not product_flow.empty and 'efficiency' in product_flow.columns:
            valid_efficiency_cats = ['Undersupplied', 'Balanced', 'Oversupplied']; efficiency_counts = product_flow[product_flow['efficiency'].isin(valid_efficiency_cats)]['efficiency'].value_counts()
        ...
        if not product_flow.empty and 'efficiency' in product_flow.columns:
             valid_cats = ['Undersupplied', 'Balanced', 'Oversupplied']; eff_group = product_flow[product_flow['efficiency'].isin(valid_cats)].groupby('efficiency', observed=False)[['current_stock', 'sales_quantity']].sum().reset_index()
        ...
        if not pareto_data.empty and all(c in pareto_data.columns for c in ['name', 'sales_quantity', 'cumulative_percentage', 'category']):
            ...
            colors = px.colors.qualitative.Plotly; valid_categories = sorted([cat for cat in pareto_data['category'].unique() if pd.notna(cat)])
            for i, category in enumerate(valid_categories):
                cat_data = pareto_data[pareto_data['category'] == category]
                if not cat_data.empty: fig6.add_trace(go.Scatter(x=cat_data['name'], y=cat_data['cumulative_percentage'], name=f"{int(category)}-{int(category + 10)}%", mode='lines+markers', line=dict(color=colors[i % len(colors)], dash='dash'), yaxis="y2"))
            fig6.update_layout(title="📊 تحليل باريتو (80/20)", xaxis_title="المنتج", yaxis_title="المبيعات", yaxis2=dict(title="النسبة التراكمية (%)", overlaying="y", side="right", range=[0, 100], showgrid=False), legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
            self.visualizations['fig6'] = fig6
        else: self.visualizations['fig6'] = go.Figure().update_layout(title="📊 تحليل باريتو (80/20)").add_annotation(text="بيانات باريتو غير متوفرة", showarrow=False)
        if not product_flow.empty and all(c in product_flow.columns for c in ['salePrice', 'sales_quantity', 'name']):
             ...
             else: self.visualizations['fig7'] = go.Figure().update_layout(title=fig7_title).add_annotation(text="لا توجد منتجات بأسعار وكميات موجبة", showarrow=False)
        ...
        if not stagnant_products_orig.empty:
             stagnant_products_for_plot = stagnant_products_orig.copy(); hover_data_dict_str = {}
             ...
             if 'current_stock' in stagnant_products_for_plot.columns: stagnant_products_for_plot['current_stock_str'] = stagnant_products_for_plot['current_stock'].round(1).astype(str).fillna('N/A'); hover_data_dict_str['المخزون'] = stagnant_products_for_plot['current_stock_str']
             ...
             if all(c in stagnant_products_for_plot.columns for c in required_cols_fig10):
                 try:
                     ...
                     else: self.visualizations['fig10'] = go.Figure().update_layout(title=fig10_title).add_annotation(text="لا توجد بيانات راكدة صالحة للعرض", showarrow=False)
                 except Exception as e_fig10: st.error(f"خطأ إنشاء الرسم 10: {e_fig10}"); self.visualizations['fig10'] = go.Figure().update_layout(title=fig10_title).add_annotation(text="خطأ إنشاء", showarrow=False)
        ...
        if isinstance(daily_sales_actual_df_fig11, pd.DataFrame) and not daily_sales_actual_df_fig11.empty:
             try:
                 df_actual_plot = daily_sales_actual_df_fig11.copy(); df_actual_plot['date'] = pd.to_datetime(df_actual_plot['date']); df_actual_plot = df_actual_plot.sort_values('date')
                 if not df_actual_plot.empty: last_actual_row = df_actual_plot.iloc[-1]; last_actual_point_details_fig11 = {'date': last_actual_row['date'], 'actual': last_actual_row['actual']}
        ...
        if isinstance(future_forecast_df_fig11, pd.DataFrame) and not future_forecast_df_fig11.empty:
            ...
            else: st.warning(f"عمود التاريخ '{DATE_COLUMN_CSV}' مفقود في بيانات التنبؤ CSV."); forecast_to_plot = pd.DataFrame()
            if FORECAST_COLUMN_CSV in forecast_to_plot.columns and not forecast_to_plot.empty:
                try:
                    forecast_to_plot = forecast_to_plot.sort_values(DATE_COLUMN_CSV); connected_forecast_df = forecast_to_plot
                    if last_actual_point_details_fig11 is not None:
                        last_actual_dt = last_actual_point_details_fig11['date']; last_actual_val = last_actual_point_details_fig11['actual']
                    ...
                    if not connected_forecast_df.empty:
                        ...
                        has_lower_ci_csv = LOWER_CI_COLUMN_CSV in connected_forecast_df.columns; has_upper_ci_csv = UPPER_CI_COLUMN_CSV in connected_forecast_df.columns
                        ...
                        # No warning if CI columns are missing, handled by load_forecast_csv
                except Exception as e_plot_forecast_fig11: st.warning(f"خطأ رسم تنبؤ fig11: {e_plot_forecast_fig11}")
            elif not future_forecast_df_fig11.empty: st.warning(f"ملف CSV يفتقد لعمود التنبؤ '{FORECAST_COLUMN_CSV}'.")
        elif last_actual_point_details_fig11 is not None: st.info("لم يتم تحميل بيانات التنبؤ من CSV لعرضها مع البيانات الفعلية.")
        ...
        if not all_dates_fig11.empty:
            ...
            except Exception as e_xaxis_fig11: st.warning(f"خطأ تحديد نطاق X لـ fig11: {e_xaxis_fig11}")
        ...
        if not sid_data.empty and all(c in sid_data.columns for c in ['product_id', sid_qty_col_fig14]):
             sales_per_product = sid_data.groupby("product_id")[sid_qty_col_fig14].sum(); sales_per_product = sales_per_product[pd.to_numeric(sales_per_product, errors='coerce').notnull() & (sales_per_product > 0)]
             if not sales_per_product.empty:
                 sales_freq = sales_per_product.value_counts().reset_index()
                 if len(sales_freq.columns) == 2: sales_freq.columns = ["الكمية المباعة", "عدد المنتجات"]; fig14_data = sales_freq.sort_values(by="عدد المنتجات", ascending=False).head(15)
                 if not fig14_data.empty: fig14_data["الكمية المباعة"] = fig14_data["الكمية المباعة"].round(1).astype(str); fig14_data["عدد المنتجات"] = fig14_data["عدد المنتجات"].astype(str); fig14 = ff.create_table(fig14_data, index=False); self.visualizations['fig14'] = fig14; self.visualizations['fig14_title'] = fig14_title_display
        ...
        if pie_data and isinstance(pie_data, dict) and 'revenue' in pie_data and not pie_data['revenue'].empty and 'profit' in pie_data and not pie_data['profit'].empty and 'color_mapping' in pie_data and isinstance(pie_data['color_mapping'], dict):
            ...
            revenue_to_plot = pie_data['revenue']; profit_to_plot = pie_data['profit']
            ...
            fig16.update_traces(hoverinfo='label+percent+value', textinfo='percent', textfont_size=11, insidetextorientation='radial'); fig16.update_layout(height=600, showlegend=False, title_text=fig16_title); self.visualizations['fig16'] = fig16
        else: self.visualizations['fig16'] = go.Figure().update_layout(title=fig16_title).add_annotation(text="بيانات الإيراد/الربح غير كافية", showarrow=False)
        ...
        if not outstanding.empty and all(c in outstanding.columns for c in ['user_id', 'outstanding_amount']):
             total_outstanding = outstanding["outstanding_amount"].sum(); outstanding_sorted = outstanding.sort_values("outstanding_amount", ascending=False); outstanding_sorted['user_id'] = outstanding_sorted['user_id'].astype(str)
             fig17 = px.bar(outstanding_sorted, x="user_id", y="outstanding_amount", color="outstanding_amount", title=fig17_title, labels={"user_id": "المورد", "outstanding_amount": "المبلغ الآجل"}, color_continuous_scale="Inferno"); fig17.update_coloraxes(colorbar_title="المبلغ الآجل"); fig17.update_layout(xaxis={'categoryorder': 'total descending', 'type': 'category'})
             fig17.add_annotation(xref="paper", yref="paper", x=0.98, y=0.98, text=f"مجموع الآجل: {round(total_outstanding,2)}", showarrow=False, font=dict(size=18, color="black"), bgcolor="rgba(211,211,211,0.7)", bordercolor="black", borderwidth=1, borderpad=4, align="right"); self.visualizations['fig17'] = fig17
    ...
    # --- مرحلة 5: تشغيل خط الأنابيب ---
    # --- تعديل: إزالة uploaded_forecast_csv من المدخلات واستخدام المسار الثابت ---
    def run_pipeline(self, forecast_csv_path):
        ...
        try:
            ...
            if self.raw_data is None or not isinstance(self.raw_data, dict) or \
               ...
               'sale_invoices_details' not in self.raw_data or self.raw_data['sale_invoices_details'].empty:
                 st.error("فشل تحميل بيانات Excel الأساسية.")
                 pipeline_success = False
            else: excel_data_loaded = True
        except Exception as e_load: st.error(f"فشل تحميل البيانات الأصلية (Excel): {e_load}"); st.exception(e_load); pipeline_success = False
        if pipeline_success:
             try:
                 # --- تعديل: استخدام المسار الثابت ---
                 csv_data_loaded = self.load_forecast_csv(forecast_csv_path)
             except Exception as e_csv_load: st.error(f"خطأ تحميل ملف CSV: {e_csv_load}"); st.exception(e_csv_load); self.forecast_data_from_csv = pd.DataFrame()
        if pipeline_success and excel_data_loaded:
             try:
                 with st.spinner("جاري معالجة بيانات Excel وتحليلها..."): processed_data_result, analytics_result = self.preprocess_and_analyze(self.raw_data)
                 self.processed_data = processed_data_result; self.analytics = analytics_result
                 ...
                 if not is_analytics_valid: st.warning("لم يتم حساب بيانات المبيعات الفعلية بنجاح.")
             except Exception as e_analyze_pipe: st.error(f"فشل تحليل بيانات Excel: {e_analyze_pipe}"); st.exception(e_analyze_pipe); pipeline_success = False
        elif not excel_data_loaded: st.error("لا يمكن إجراء التحليل بسبب فشل تحميل Excel."); pipeline_success = False
        if pipeline_success:
             try:
                 if isinstance(self.analytics, dict) and self.analytics:
                     with st.spinner("جاري إنشاء الرسوم البيانية..."): self.generate_visualizations()
                     if not self.visualizations: st.warning("لم يتم إنشاء أي رسوم بيانية.")
        ...
        else: st.error("--- فشل خط أنابيب البيانات ---")

    # --- مرحلة 6: عرض لوحة التحكم ---
    def display_dashboard(self):
        st.title('📊 لوحة تحليل المخزون والمبيعات والتنبؤات')
        def get_fig(fig_key, title="N/A"):
            fig = self.visualizations.get(fig_key)
            if fig is None: fig = go.Figure().update_layout(title=f"{title}: بيانات غير متوفرة", xaxis={'visible': False}, yaxis={'visible': False}, annotations=[{'text': "بيانات غير متوفرة", 'xref': "paper", 'yref': "paper",'showarrow': False, 'font': {'size': 16}}])
            return fig
        ...
        with col14:
             fig14_title = self.visualizations.get('fig14_title', "جدول تكرار كميات البيع"); st.subheader(fig14_title)
        ...
        if not products_df_disp.empty and all(c in products_df_disp.columns for c in ['id', 'name', 'quantity']):
            ...
            if not product_flow_df_disp.empty and all(c in product_flow_df_disp.columns for c in ['product_id', 'sales_quantity']): current_stock_display = current_stock_display.merge(product_flow_df_disp[['product_id', 'sales_quantity']], on='product_id', how='left').fillna({'sales_quantity': 0})
            ...
            if all(col in current_stock_display.columns for col in display_cols_table):
                try:
                    ...
                    if 'sales_quantity' in display_cols_table: column_config_dict["sales_quantity"] = st.column_config.NumberColumn("إجمالي المبيعات", format="%.1f")
            ...
            else: st.warning("الأعمدة المطلوبة لعرض جدول المخزون غير متوفرة.")
        else: st.warning("لا توجد بيانات منتجات لعرض جدول المخزون.")

# --- التشغيل الرئيسي للداشبورد ---
if __name__ == "__main__":
    # --- إزالة أداة رفع الملفات ---
    # st.sidebar.header("تحميل ملف التنبؤات (CSV)")
    # uploaded_forecast_csv_file = st.sidebar.file_uploader(...) # تم الحذف
    # st.sidebar.markdown("---")
    # --- نهاية الإزالة ---
    ...
    # --- إضافة معلومة عن مسار CSV الثابت ---
    st.sidebar.caption(f"التنبؤات CSV: `{FORECAST_CSV_PATH}`")
    st.sidebar.markdown("_تأكد من وجود هذه الملفات في المسارات المحددة وتعديل `FORECAST_CSV_PATH` في الكود إذا لزم الأمر._")
    # --- نهاية الإضافة ---

    pipeline = DataPipeline()
    try:
        # --- تمرير المسار الثابت إلى run_pipeline ---
